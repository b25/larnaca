...
namespace envoy.controller.Cache
{
    ...
    /// All the following considerations apply to this cache.
    /// 
    ...
    public class SnapshotCache : ISnapshotCache
    {
        ...
        {
            var nodes = _nodeInfos.Keys.ToList();
        ...
        public void SetSnapshot(string node, Snapshot snapshot)
        {
            ...
            _logger.InfoF($"Setting new snapshot with versions cluster: {snapshot.Clusters.Version}, endpoints: {snapshot.Endpoints.Version}, listeners: {snapshot.Listeners.Version}, routes: {snapshot.Routes.Version}");
            ...
            lock (info.Lock)
            {
                ...
                requestsToResolve = info.PendingResponses
                    ...
                    .OrderBy(r => TypeStrings.GetPriority(r.kv.Value.Request.TypeUrl)) // This ensures the responses will be sent in following order CDS, EDS, LDS, RDS
                    .ToArray();
            ...
            foreach (var request in requestsToResolve)
            {
                ...
                _logger.DebugF($"-> Responding from new snapshot TypeUrl: {request.response.TypeUrl}");
            ...
            string version)
        {
                ...
                request.ResourceNames.Intersect(resources.Keys).Count() < resources.Count)
            {
                ...
                _logger.DebugF($"Skipping response due to missing resources in ads mode for request TypeUrl: {request.TypeUrl}, Version: {request.VersionInfo}, Nonce: {request.ResponseNonce}");
                return null;
            }
